...
- (NSMutableDictionary *)defaultHeaderParams
{
    ...
    if (self.cookie.length) {
        params[@"Cookie"] = [NSString stringWithFormat:@"MCSESSID=%@;", self.cookie];
    }
    ...
    if (self.userAgent.length) {
        ...
        params[@"User-Agent"] = [Utility userAgentString];
    }
...
{
    NSString *url = nil;
    if (self.imageUploadURL.length) {
        url = self.imageUploadURL;
    }
    else {
        url = [self getAddressWithAPI:API_UPLOADIMAGES];
    }
